...
class LivraisonManager{
	...
	public function getLivraisonsByIdProjetByLimit($idProjet, $begin, $end){
        $livraisons = array();
        $query = $this->_db->prepare('SELECT * FROM t_livraison WHERE idProjet=:idProjet ORDER BY id DESC LIMIT '.$begin.', '.$end);
        $query->bindValue(':idProjet', $idProjet);
        $query->execute();
        while($data = $query->fetch(PDO::FETCH_ASSOC)){
            $livraisons[] = new Livraison($data);
	...
	public function getLivraisonsByIdFournisseurByLimit($idFournisseur, $begin, $end){
        $livraisons = array();
        $query = $this->_db->prepare('SELECT * FROM t_livraison WHERE idFournisseur=:idFournisseur ORDER BY id DESC LIMIT '.$begin.', '.$end);
        $query->bindValue(':idFournisseur', $idFournisseur);
        $query->execute();
        while($data = $query->fetch(PDO::FETCH_ASSOC)){
            $livraisons[] = new Livraison($data);
	...
	//new functions
	//add to LivraisonManager class on 
	//http://merlatrav.esy.es - status : done
