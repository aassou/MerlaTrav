...
class ReglementFournisseurManager{
	...
	//http://merlatrav.esy.es - status : done
	...
	public function getReglementNumber(){
		$query = $this->_db->query('SELECT count(id) AS nombreReglements FROM t_reglement_fournisseur');
        $data = $query->fetch(PDO::FETCH_ASSOC);
        $query->closeCursor();
	...
	public function getReglementsNumberByFournisseur($idFournisseur){
        $query = $this->_db->prepare('SELECT count(id) AS nombreReglements 
        FROM t_reglement_fournisseur WHERE idFournisseur=:idFournisseur');
		$query->bindValue(':idFournisseur', $idFournisseur);
        $query->execute();
	...
	public function getReglementsByLimit($begin, $end){
        $reglementFournisseurs = array();
        $query = $this->_db->query('SELECT * FROM t_reglement_fournisseur ORDER BY id DESC LIMIT '.$begin.' ,'.$end);
        while ($data = $query->fetch(PDO::FETCH_ASSOC)) {
            $reglementFournisseurs[] = new ReglementFournisseur($data);
        }
        $query->closeCursor();
